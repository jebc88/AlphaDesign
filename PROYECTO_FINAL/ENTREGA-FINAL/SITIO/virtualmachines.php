...
if(isset($_POST['update'])){
    if($_POST['estado']==1){
        $update = database::deactivateVM($_POST['idvm'],$id);
        $vms = database::getActiveVM();
        $dir = 'local';
        //echo $dir; 
        //rmdir($dir);
        foreach(glob($dir . "/*") as $archivos_dir)
        {
        //echo $archivos_dir;
        ...
        $update = database::activateVM($_POST['idvm']);
        $vms = database::getActiveVM();
    }
}
...
<head>
    ...
    <script language="Javascript">
        ...
        function openBox(){
            ...
            function openBox2(){
            url = "respaldo.php";
            newwindow=window.open(url,'name','height=800,width=400');
            if (window.focus) {newwindow.focus()}
        }
...
<div id="content-top">
    <div id="content-top-inner">
        <div class="container">
            <div class="row">
                <div class="span12">
                    <h1><?php if($vms=='0'){echo 'NO EXISTEN MAQUINAS VIRTUALES ACTIVAS';}else{echo 'Máquinas Virtuales Activas';}?></h1>
                </div>
            </div>
        </div>
    </div>
            ...
            <table border="1px">
                ...
                <tr> <?php foreach($detail as $data){ ?>
                    ...
                    <form method="post" action="virtualmachines.php" name="vms">
                        ...
                        <input name="estado" type="hidden" value="<?php echo $data['estado'];?>">
                        <td><input type="submit"  name="backup" onclick="openBox2()" value="Respaldar"></td>
                        <td><input type="submit"  name="restore" onclick="openBox()" value="Restaurar"></td>
                        <span><td><input type="submit"  name="update" value="<?php  if($data['estado']==1){echo 'Desactivar';}else{echo 'Activar';} ?>"></td></span>
                    </form>
                </tr>
                <?php } ?>
                </tbody>
